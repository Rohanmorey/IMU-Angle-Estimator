...
float magX_min = 4.20;
float magX_max = 33.75;
float magY_min = -41.55;
float magY_max = -11.85;
float magZ_min = -65.70;
float magZ_max = -26.70;
...
void loop() {
  if (imu.dataReady()) {
    imu.getAGMT(); // Read accelerometer, gyroscope, magnetometer, and temperature data
    ...
    float mx_corr = mx - magX_offset;
    float my_corr = my - magY_offset;
    float mz_corr = mz - magZ_offset;
    ...
    Madgwick(gx, gy, gz, ax, ay, az, mx_corr, my_corr, mz_corr, 0.5);
